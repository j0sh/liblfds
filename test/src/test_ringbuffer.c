...
/****************************************************************************/
void test_lfds601_ringbuffer( void )
{
  printf( "\n"
          "Ringbuffer Tests\n"
          "================\n" );
...
void ringbuffer_test_reading( void )
{
  ...
  */
  ...
  lfds601_ringbuffer_new( &rs, 1000000, NULL, NULL );

  for( loop = 0 ; loop < 1000000 ; loop++ )
  {
    lfds601_ringbuffer_get_write_element( rs, &fe, NULL );
    lfds601_freelist_set_user_data_in_element( fe, (void *) (lfds601_atom_t) loop );
    lfds601_ringbuffer_put_write_element( rs, fe );
  }
  ...
  lfds601_ringbuffer_query( rs, LFDS601_RINGBUFFER_QUERY_VALIDATE, (void *) &vi, (void *) dvs );
...
thread_return_t CALLING_CONVENTION ringbuffer_test_thread_simple_reader( void *ringbuffer_test_reading_state )
{
  ...
  lfds601_ringbuffer_get_read_element( rtrs->rs, &fe );
  lfds601_freelist_get_user_data_from_element( fe, (void **) &prev_user_data );
  lfds601_ringbuffer_put_read_element( rtrs->rs, fe );

  rtrs->read_count++;

  while( lfds601_ringbuffer_get_read_element(rtrs->rs, &fe) )
  {
    lfds601_freelist_get_user_data_from_element( fe, (void **) &user_data );
    lfds601_ringbuffer_put_read_element( rtrs->rs, fe );
...
void ringbuffer_test_writing( void )
{
  ...
  */
  ...
  lfds601_ringbuffer_new( &rs, 100000, NULL, NULL );
  ...
  {
    (rtws+loop)->rs = rs;
    (rtws+loop)->write_count = (lfds601_atom_t) loop << (sizeof(lfds601_atom_t)*8-8);
  }
  ...
  while( dvs[0] == LFDS601_VALIDITY_VALID and dvs[1] == LFDS601_VALIDITY_VALID and dvs[2] == LFDS601_VALIDITY_VALID and lfds601_ringbuffer_get_read_element(rs, &fe) )
  {
    lfds601_freelist_get_user_data_from_element( fe, (void *) &user_data );
    ...
    {
      dvs[0] = LFDS601_VALIDITY_INVALID_TEST_DATA;
      lfds601_ringbuffer_put_read_element( rs, fe );
      break;
    }
...
thread_return_t CALLING_CONVENTION ringbuffer_test_thread_simple_writer( void *ringbuffer_test_writing_state )
{
  ...
  {
    lfds601_ringbuffer_get_write_element( rtws->rs, &fe, NULL );
    lfds601_freelist_set_user_data_in_element( fe, (void *) (lfds601_atom_t) (rtws->write_count++) );
    lfds601_ringbuffer_put_write_element( rtws->rs, fe );
  }
...
void ringbuffer_test_reading_and_writing( void )
{
  ...
  */
  ...
  {
    (rtrws+loop)->rs = rs;
    (rtrws+loop)->counter = (lfds601_atom_t) loop << (sizeof(lfds601_atom_t)*8-8);
    (rtrws+loop)->cpu_count = cpu_count;
    (rtrws+loop)->error_flag = LOWERED;
    (rtrws+loop)->per_thread_counters = malloc( sizeof(lfds601_atom_t) * cpu_count );
...
thread_return_t CALLING_CONVENTION ringbuffer_test_thread_reader_writer( void *ringbuffer_test_reading_and_writing_state )
{
  ...
  while( time(NULL) < start_time + 10 )
  {
    lfds601_ringbuffer_get_write_element( rtrws->rs, &fe, NULL );
    lfds601_freelist_set_user_data_in_element( fe, (void *) (lfds601_atom_t) (rtrws->counter++) );
    lfds601_ringbuffer_put_write_element( rtrws->rs, fe );

    lfds601_ringbuffer_get_read_element( rtrws->rs, &fe );
    lfds601_freelist_get_user_data_from_element( fe, (void *) &user_data );
    ...
    lfds601_ringbuffer_put_read_element( rtrws->rs, fe );
  }
