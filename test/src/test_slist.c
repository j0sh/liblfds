...
void test_lfds601_slist( void )
{
  ...
  */
  ...
  lfds601_slist_new( &stss.ss, NULL, NULL );
  ...
  for( loop = 0 ; loop < cpu_count ; loop++ )
    abstraction_thread_start( &thread_handles[loop], loop, lfds601_slist_internal_thread_head_writer, &stss );

  for( loop = 0 ; loop < cpu_count ; loop++ )
    abstraction_thread_wait( thread_handles[loop] );

  lfds601_slist_delete( stss.ss );
  ...
  lfds601_slist_new( &stss.ss, NULL, NULL );
  stss.iteration_modulo = 1;
  stss.se = lfds601_slist_new_head( stss.ss, (void *) NULL );
  stss.duration = 1;

  thread_handles = (thread_state_t *) malloc( sizeof(thread_state_t) * cpu_count * 1 );

  for( loop = 0 ; loop < cpu_count ; loop++ )
    abstraction_thread_start( &thread_handles[loop], loop, lfds601_slist_internal_thread_after_writer, &stss );
  ...
  for( loop = 0 ; loop < 10 ; loop++ )
    lfds601_slist_new_head( stss.ss, (void *) 0 );

  thread_handles = (thread_state_t *) malloc( sizeof(thread_state_t) * cpu_count * 1 );

  for( loop = 0 ; loop < cpu_count ; loop++ )
    abstraction_thread_start( &thread_handles[loop], loop, lfds601_slist_internal_thread_traverser, &stss );
  ...
  {
    abstraction_thread_start( &thread_handles[loop], loop, lfds601_slist_internal_thread_head_writer, &stss );
    abstraction_thread_start( &thread_handles[loop+cpu_count], loop, lfds601_slist_internal_thread_traverser, &stss );
  }
  ...
  for( loop = 0 ; loop < cpu_count ; loop++ )
    abstraction_thread_start( &thread_handles[loop], loop, lfds601_slist_internal_thread_deleter_traverser, &stss );
  ...
  {
    abstraction_thread_start( &thread_handles[loop], loop, lfds601_slist_internal_thread_head_writer, &stss );
    abstraction_thread_start( &thread_handles[loop+cpu_count], loop, lfds601_slist_internal_thread_deleter_traverser, &stss );
  }
  ...
  {
    abstraction_thread_start( &thread_handles[loop], loop, lfds601_slist_internal_thread_head_writer, &stss );
    abstraction_thread_start( &thread_handles[loop+cpu_count], loop, lfds601_slist_internal_thread_after_writer, &stss );
    abstraction_thread_start( &thread_handles[loop+cpu_count*2], loop, lfds601_slist_internal_thread_traverser, &stss );
    abstraction_thread_start( &thread_handles[loop+cpu_count*3], loop, lfds601_slist_internal_thread_deleter_traverser, &stss );
  }
...
/****************************************************************************/
thread_return_t CALLING_CONVENTION lfds601_slist_internal_thread_head_writer( void *lfds601_slist_thread_start_state )
{
  struct lfds601_slist_thread_start_state
    *stss;
  ...
  assert( lfds601_slist_thread_start_state != NULL );

  stss = (struct lfds601_slist_thread_start_state *) lfds601_slist_thread_start_state;

  time( &start_time );

  while( time(NULL) < start_time + stss->duration )
    if( lfds601_slist_new_head(stss->ss, (void *) 0) )
      count++;
...
/****************************************************************************/
thread_return_t CALLING_CONVENTION lfds601_slist_internal_thread_after_writer( void *lfds601_slist_thread_start_state )
{
  struct lfds601_slist_thread_start_state
    *stss;
  ...
  while( time(NULL) < start_time + stss->duration )
    if( lfds601_slist_new_next(stss->se, (void *) 0) )
      count++;
...
/****************************************************************************/
thread_return_t CALLING_CONVENTION lfds601_slist_internal_thread_traverser( void *lfds601_slist_thread_start_state )
{
  struct lfds601_slist_thread_start_state
    *stss;
  ...
  lfds601_slist_get_head( stss->ss, &se );
...
/****************************************************************************/
thread_return_t CALLING_CONVENTION lfds601_slist_internal_thread_deleter_traverser( void *lfds601_slist_thread_start_state )
{
  struct lfds601_slist_thread_start_state
    *stss;
  ...
  while( time(NULL) < start_time + stss->duration )
  {
    ...
    {
      lfds601_slist_delete_element( stss->ss, se );
      count++;
    }
