...
void internal_display_test_result( unsigned int number_name_dvs_pairs, ... )
{
  ...
  {
    name = va_arg( va, char * );
    dvs = va_arg( va, enum lfds601_data_structure_validity );

    if( dvs != LFDS601_VALIDITY_VALID )
    {
      passed_flag = LOWERED;
      break;
    }
...
void internal_display_data_structure_validity( enum lfds601_data_structure_validity dvs )
{
  ...
  switch( dvs )
  {
    case LFDS601_VALIDITY_VALID:
      string = "valid";
    break;

    case LFDS601_VALIDITY_INVALID_LOOP:
      string = "invalid - loop detected";
    break;

    case LFDS601_VALIDITY_INVALID_MISSING_ELEMENTS:
      string = "invalid - missing elements";
    break;

    case LFDS601_VALIDITY_INVALID_ADDITIONAL_ELEMENTS:
      string = "invalid - additional elements";
    break;

    case LFDS601_VALIDITY_INVALID_TEST_DATA:
